...
const Signup = () => {
  ...
  const validateData = ()=>{
    ...
    if(!email || (!isEmail(email))) return setFunction(false, "Please type a valid email address")
    if(!password || !confirmPassword) return setFunction(false, "Please type the password and confirm password");
    if(password != confirmPassword) return setFunction(false, " password and confirm password must be same");
    if(!isStrongPassword(password)) return setFunction(false, "Password must contain uppercase, lowercase, number, and special character");
    if(!adminValidator) return setFunction(false, "Enter the admin secret to create the account")
    return true;    
} 
          ...
          <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            <form  className="space-y-6">
              ...
              legendText={"Admin secret"}
               typeText={'password'} 
              setInput={setAdminValidator}
              inputValue = {adminValidator}/>
              <div>
                <button
                  type="submit"
